		...
		<form id="save_menu" method="post">
			<div class="content">
				<div class="column">
					...
					<input value="${menu.menuid }" type="hidden" name="menuid"> 
				</div>
					...
					<tbody>
						...
						<td class="kv-label">上级资源</td>
							...
							<td class="kv-content"><input class="easyui-textbox"
								type="text" name="name" data-options="required:true" value="${menu.name}" style="height:35px;" /></td>
						...
						<tr>
							...
							<td class="kv-content"><input class="easyui-textbox"
								type="text" name="permission" data-options="required:true" value="${menu.permission}"  style="height:35px;"/></td>
								
								<td class="kv-label">地址</td>
							<td class="kv-content"><input class="easyui-textbox"
								type="text" name="url" data-options="required:true" value="${menu.url}"  style="height:35px;"/></td>
						 
						</tr>
						<tr>
						<td class="kv-label">图标</td>
							<td class="kv-content"> 
							<input class="easyui-filebox" name=menu_icon value="${menu.menu_icon}" data-options="prompt:'请选择文件...', buttonText:' 选 择 '"   style="width:80%;height:35px;">
							</td>
								 <td class="kv-label">排序</td>
							<td class="kv-content"  ><input class="easyui-textbox"
								type="text" name="sort" data-options="required:true" value="${menu.sort}"  style="height:35px;"/></td>
								
						</tr>
						<tr>
						<td class="kv-label">就否显示</td>
							<td class="kv-content"  ><input type="radio"
								name="isshow" value="1"  <c:if test="${menu.isshow eq 1 }">checked="checked" </c:if>
								class="easyui-radiobox"> 显示 <input type="radio"
								name="isshow" value="0" <c:if test="${menu.isshow eq 0 }">checked="checked" </c:if> class="easyui-radiobox"> 隐藏</td>
							<td class="kv-label">类型</td>
							<td class="kv-content"  ><input type="radio"
								name="type" value="1"  <c:if test="${menu.type eq 1 }">checked="checked" </c:if> 
								class="easyui-radiobox"> 模块 <input type="radio"
								name="type" value="0" <c:if test="${menu.type eq 0 }">checked="checked" </c:if> class="easyui-radiobox"> 菜单</td>
						</tr>
						 <tr>
							<td class="kv-label">摘要</td>
							<td class="kv-content" colspan="3">
							<textarea rows="5" cols="50" name="remarks">${menu.remarks}</textarea></td>
	...
	$(function(){ 
		$("#parent_id").combotree({
			...
			onLoadSuccess:function(node, data){
				$("#parent_id").combotree('setValue',"${menu.parent_id}");
			}
		});
	})
