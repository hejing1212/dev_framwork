...
<body>
	<div class="container">
		<form id="save_menu" method="post">
			<div class="content">
				<div class="column">
					<span class="current">菜单信息</span> <input value="${menu.menuid }"
				...
				<table class="kv-table">
					<tbody>
						<tr>
							...
							<td class="kv-content"><select class="easyui-combotree"
								name="parent_id" id="parent_id" data-options="required:true"
							...
							<td class="kv-content"><input class="easyui-textbox"
								...
								value="${menu.name}" style="height: 35px;" /></td>
						...
						<tr>
							...
							<td class="kv-content"><input type="radio" name="isshow"
								value="1"
								<c:if test="${menu.isshow eq 1 }">checked="checked" </c:if>
								class="easyui-radiobox"> 显示 <input type="radio"
								name="isshow" value="0"
								<c:if test="${menu.isshow eq 0 }">checked="checked" </c:if>
								class="easyui-radiobox"> 隐藏</td>
							<td class="kv-label">类型</td>
							<td class="kv-content" ><input type="radio"
								name="type" value="1"
								<c:if test="${menu.type eq 1 }">checked="checked" </c:if>
								class="easyui-radiobox"> 模块 <input type="radio"
								name="type" value="0"
								<c:if test="${menu.type eq 0 }">checked="checked" </c:if>
						...
						<tr>
							...
							<td class="kv-content" colspan="3"><textarea rows="5"
									cols="50" name="remarks">${menu.remarks}</textarea></td>
