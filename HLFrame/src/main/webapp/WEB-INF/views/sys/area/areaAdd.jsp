		...
		<form id="save_menu" method="post">
			<div class="content">
				...
				<table class="kv-table">
					<tbody>
						<tr>
						<td class="kv-label">上级地区名称</td>
							<td class="kv-content">
							<select class="easyui-combotree" name="areaTree"  id="areaTree" data-options="required:true"   style="height:35px;width:40%">
							</select>
                             </td>
	...
	$(function(){ 
		$("#areaTree").combotree({
			...
			onBeforeExpand:function(row){    //每次展开前都会调用
	           ...
	           $("#areaTree").treegrid("options").url = url;  
	           return true;  //返回false表示停止展开节点    
	          },
	        onExpand : function(row){ //每次展后都会调用;传入的row已经包含了 children
	           var children = $("#areaTree").treegrid('getChildren',row.id);
	           if(children.length<=0){ 
	               row.leaf=true;
	               $("#areaTree").treegrid('refresh', row.id);
	           }
	        },
