					...
					<tbody>
						<tr>
							...
							<input type="hidden" name="id" value="${item.id }" />
					         <input type="hidden" name="dictCode" value="${item.dictCode}" />
							<input class="easyui-textbox" value="${item.itemName }"
								type="text" name="itemName" data-options="required:true,validType:['stringCheck','length[1,32]'],missingMessage:'请输入键值名称'" /></td>

							<td class="kv-label">键值</td>
							<td class="kv-content"><input class="easyui-textbox" value="${item.itemValue }"
						...
						<tr>
							...
							<td class="kv-content"  colspan="3">
							<textarea rows="5" cols="50" name="remarks">${item.remarks }</textarea></td>
					    </tr>
		...
		$('#save_user_form').form('submit', {
			...
			success : function(data) {
				...
				if (data.code == '1') {
					...
					$.messager.alert('添修失败', data.msg, 'error');
				}
			}
		});
	}
