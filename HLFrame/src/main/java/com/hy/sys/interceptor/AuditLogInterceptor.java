...
public class AuditLogInterceptor extends EmptyInterceptor {
	...
	public void onDelete(Object entity, Serializable id, Object[] state, String[] propertyNames, Type[] types) {
		...
		logEntity.setOpt_content("表：" + HibernateUtil.getTableName(entity.getClass()) + ",表名："
				+ HibernateUtil.getTableComment(entity.getClass()) + "操作记录：" + sb.toString() + "操作人："
				+ user.getUsername() + "");
		...
		try {
			...
			if (user != null) {
				...
				if (previousState == null) {
					...
					Object dben = jdbcTemplate.queryForObject(sql, new Object[] { id },
				...
				if (previousState != null) {
					...
					for (int i = 0; i < propertyNames.length; i++) {
						boolean isadd = false;
						if (currentState[i] == null && !"".equals(currentState[i]) && previousState[i] != null
								&& !"".equals(previousState[i])) {
							isadd = true;
						} else if (currentState[i] != null && !"".equals(currentState[i]) && previousState[i] == null
								&& !"".equals(previousState[i])) {
							isadd = true;
						} else if (currentState[i] != null && !"".equals(currentState[i]) && previousState[i] != null
								&& !"".equals(previousState[i]) && !currentState[i].equals(previousState[i])) {
							isadd = true;
						}
						logEntity.setOpt_type(2);
						...
						if (isadd) {
							if (types[i] instanceof TimestampType) {
								...
								if (previousState[i] != null) {
									str = formatter.format(previousState[i]);
								} else {
									str = "";
								}
